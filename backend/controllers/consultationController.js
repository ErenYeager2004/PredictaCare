...
export const verifyConsultationPayment = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: "Payment verified. Admin will assign a doctor shortly.",
    });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
...
export const getUserConsultations = async (req, res) => {
  try {
    const userId = req.userId;
    const consultations = await Consultation.find({ userId }).sort({
      createdAt: -1,
    });
...
export const getCallToken = async (req, res) => {
  try {
    ...
    if (consultation.status !== "accepted") {
      return res.status(400).json({
        success: false,
        message: "Consultation not yet accepted by doctor",
      });
    }
...
export const completeConsultation = async (req, res) => {
  try {
    const { consultationId, doctorNotes, prescription, followUpRequired } =
      req.body;
...
export const endConsultationCall = async (req, res) => {
  try {
...
export const assignDoctor = async (req, res) => {
  try {
    const { consultationId, doctorId, doctorName, doctorImage, speciality } =
      req.body;
...
export const getDoctorConsultations = async (req, res) => {
  try {
    ...
    const consultations = await Consultation.find({ doctorId: docId }).sort({
      scheduledDate: 1,
    });
    return res.json({ success: true, consultations });
  } catch (err) {
...
export const doctorRespond = async (req, res) => {
  try {
    ...
    const update =
      action === "accept"
        ? { status: "accepted", callEnded: false, }
        : {
            status: "rejected",
            rejectionReason: rejectionReason || "Doctor unavailable",
          };
    ...
    return res.json({
      success: true,
      message:
        action === "accept" ? "Consultation accepted" : "Consultation rejected",
    });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
