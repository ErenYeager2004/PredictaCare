...
if not os.path.exists("predictionModel"):
    ...
    print("   cd backend")
    print("   python predictionModel/pcos/train_pcos_lite.py")
    sys.exit(1)
...
MODEL_PATH     = "predictionModel/lite_models/pcos_lite.pkl"
COLUMNS_PATH   = "predictionModel/lite_models/pcos_lite_columns.pkl"
...
# Outlier caps for hormonal values (not used in free tier but good practice)
MIN_RECALL = 0.80
...
def evaluate(model, X_test, y_test, threshold):
    ...
    print(f"\n{'='*55}")
    print("  PCOS FREE TIER — Final Evaluation")
    print(f"{'='*55}")
...
model = XGBClassifier(
    ...
    min_child_weight  = 3,      # higher for small dataset — prevents overfitting
...
print("\n PCOS free tier training complete.")
print(f"   Free tier: {len(FEATURES)} self-reported features")
print(f"   Premium DNN: 18 features including AMH, LH, FSH + ultrasound")
