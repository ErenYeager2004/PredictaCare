    ...
    >
      ...
      {isTampered && (
        ...
        >
          ...
          DATA TAMPERING DETECTED — This prediction's result has been altered
          after blockchain sealing
        </div>
      )}
      ...
      <div style={{ padding: "18px 20px" }}>
              ...
              >
                ...
                <span
                  style={{
                    ...
                    borderRadius: 999,
                    background:
                      prediction.tier === "premium" ? "#fffbeb" : "#f0fdf4",
                    color:
                      prediction.tier === "premium" ? "#d97706" : "#16a34a",
                    fontWeight: 600,
                  }}
                >
        ...
        {isTampered && expanded && (
          ...
          >
            ...
            >
              The hash of the current data does not match the hash stored on
              Ethereum Sepolia (Block #{prediction.blockNumber}). This is
              cryptographic proof that the data was altered after it was sealed.
              Contact{" "}
              <a
                href="mailto:adminPredictaCare@gmail.com"
...
export default function MyPredictions() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const { data } = await axios.get(
          `${BACKEND_URL}/api/predictions/history`,
          {
            headers: { token },
          },
        );
  ...
  const handleDownload = (prediction) => {
    ...
    doc.text("PredictaCare — Prediction Certificate", 105, 20, {
      align: "center",
    });
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text(`Generated: ${date.toLocaleString()}`, 105, 28, {
      align: "center",
    });
    ...
    doc.setFontSize(10);
    doc.text(
      `Disease: ${DISEASE_LABEL[prediction.disease] || prediction.disease}`,
      15,
      52,
    );
    ...
    if (prediction.blockchainTxHash) {
      ...
      doc.text(`Hash: ${prediction.blockchainHash}`, 15, 124, {
        maxWidth: 180,
      });
    }
    ...
    doc.text(
      "AI screening only — not a medical diagnosis. Consult a qualified healthcare professional.",
      105,
      282,
      { align: "center" },
    );
    doc.save(
      `PredictaCare_${prediction.disease}_${date.toISOString().slice(0, 10)}.pdf`,
    );
  };
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: "linear-gradient(135deg,#4F5FE0,#5F6FFF,#7C8BFF)",
      ...
      <div
        style={{ maxWidth: 900, margin: "-28px auto 48px", padding: "0 24px" }}
      >
        {/* ── Stats ─────────────────────────────────────────────────────── */}
        <div
          style={{
        ...
        >
          {[
            { label: "Total Predictions", value: total, color: "#5F6FFF" },
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                background: filter === f.key ? "#5F6FFF" : "#fff",
