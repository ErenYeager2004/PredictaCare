...
const DISEASE_CONFIG = {
  heart: {
    ...
    color: '#ef4444', lightBg: '#fff1f2', border: '#fecdd3',
  },
  diabetes: {
  ...
  pcos: {
    ...
    color: '#a855f7', lightBg: '#fdf4ff', border: '#e9d5ff',
  },
  stroke: {
    label: 'Stroke', emoji: '🧠',
    ...
    color: '#14b8a6', lightBg: '#f0fdfa', border: '#99f6e4',
  },
}
...
const SAMPLE_PREVIEW = [
  { record_id: 'PC-000001', prediction_result: 'HIGH',     probability: 0.87, tier: 'premium', blockchain_verified: true  },
  { record_id: 'PC-000002', prediction_result: 'LOW',      probability: 0.14, tier: 'free',    blockchain_verified: true  },
  { record_id: 'PC-000003', prediction_result: 'MODERATE', probability: 0.52, tier: 'free',    blockchain_verified: false },
  { record_id: 'PC-000004', prediction_result: 'HIGH',     probability: 0.91, tier: 'premium', blockchain_verified: true  },
  { record_id: 'PC-000005', prediction_result: 'LOW',      probability: 0.09, tier: 'free',    blockchain_verified: false },
]

const DatasetDetail = () => {
  const { disease } = useParams()
  const navigate = useNavigate()
  const { rToken, backendUrl, userData, createOrder, verifyPayment } = useContext(ResearchContext)
  ...
  const bcPct = dataInfo
    ? Math.round((dataInfo.blockchainVerified / dataInfo.totalRecords) * 100) || 0
    : 0
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          ...
          if (result.success) {
            toast.success('Payment successful! Check your email for the download token.')
            navigate('/my-orders')
          } else {
  ...
  return (
    <div className="p-6 sm:p-10">
      ...
      >
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">{cfg.label} Dataset</h1>
          <p className="text-gray-500 text-sm mt-1">
            {dataInfo?.totalRecords?.toLocaleString() ?? '—'} anonymised prediction records
          </p>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          ...
          {tab === 'overview' && (
            <div className="flex flex-col gap-5">
              <p className="text-gray-600 leading-relaxed text-sm">{cfg.desc}</p>

              {dataInfo && (
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { label: 'Total Records', value: dataInfo.totalRecords?.toLocaleString() },
                    { label: 'BC Verified',   value: dataInfo.blockchainVerified?.toLocaleString() },
                    { label: 'High Risk',     value: dataInfo.riskBreakdown?.HIGH?.toLocaleString() },
                  ].map((s) => (
              ...
              <div className="bg-[#eef0ff] rounded-xl p-4 flex gap-3 text-sm text-gray-700 leading-relaxed">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#5F6FFF" strokeWidth="2" className="flex-shrink-0 mt-0.5">
                  <circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>
                </svg>
                All records are <strong className="mx-1">fully anonymised</strong> — no names, emails, or identifiable information.
                Data comes only from users who gave explicit research consent on PredictaCare.
              </div>
            </div>
          )}
          ...
          {tab === 'preview' && (
            <div className="overflow-x-auto rounded-xl border border-gray-200">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 text-gray-500 uppercase tracking-wide">
                    {['Record ID', 'Disease', 'Result', 'Probability', 'Tier', 'Blockchain'].map((h) => (
                ...
                <tbody>
                  {preview.map((r, i) => (
                    <tr key={i} className="border-t border-gray-100 hover:bg-gray-50">
                      <td className="px-4 py-3 font-mono text-gray-600">{r.record_id}</td>
                      <td className="px-4 py-3 capitalize">{disease}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full font-semibold text-xs ${
                          r.prediction_result === 'HIGH'     ? 'bg-red-50 text-red-600'    :
                          r.prediction_result === 'MODERATE' ? 'bg-orange-50 text-orange-600' :
                          'bg-green-50 text-green-600'
                        }`}>
                          {r.prediction_result}
                        </span>
                      </td>
                      <td className="px-4 py-3 font-mono">{(r.probability * 100).toFixed(1)}%</td>
              ...
              <div className="bg-gray-50 px-4 py-2.5 text-xs text-gray-400 border-t border-gray-100">
                Showing 5 sample records · Personal identifiers removed · Full dataset delivered after purchase
              </div>
            </div>
          )}
        ...
        <div>
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 sticky top-20">
            <h3 className="font-semibold text-gray-900 mb-4">Purchase Dataset</h3>

            {/* Record count */}
            <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-2">Records</p>
                ...
                >
                  <div className="flex items-center gap-2">
                    <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                      selectedTier.records === t.records ? 'border-[#5F6FFF] bg-[#5F6FFF]' : 'border-gray-300'
                    }`}>
                      {selectedTier.records === t.records && (
                        <div className="w-1.5 h-1.5 bg-white rounded-full" />
                      )}
                    </div>
                    <span className="font-medium">{t.records.toLocaleString()} records</span>
                    <span className="text-xs text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded">{t.label}</span>
                  </div>
            ...
            {/* Format */}
            <p className="text-xs text-gray-500 uppercase tracking-wide font-semibold mb-2">Format</p>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-4 text-sm border border-gray-100">
              <div className="flex justify-between text-gray-600 mb-1">
                <span>Records</span><span className="font-semibold">{selectedTier.records.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-gray-600 mb-1">
                <span>Format</span><span className="font-semibold uppercase">{format}</span>
              </div>
              <div className="flex justify-between text-gray-600 mb-2">
                <span>Token validity</span><span className="font-semibold">30 days</span>
              </div>
              <div className="border-t border-gray-200 pt-2 flex justify-between font-bold text-gray-900">
                <span>Total</span>
                <span className="text-lg text-[#5F6FFF]">₹{selectedTier.price}</span>
              </div>
            </div>
